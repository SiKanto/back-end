    ...
    try {
        ...
        if (!userId || !destinationId || !ticketQuantity) {
            return h
                .response({
                    message:
                        "User, destination, and ticket quantity are required.",
                })
                .code(400);
        }
        ...
        if (!user || !destination) {
            return h
                .response({ message: "User or Destination not found" })
                .code(404);
        }
        ...
        await ticket.save();
        return h
            .response({ message: "Ticket successfully booked", ticket })
            .code(201);
    } catch (err) {
        return h
            .response({
                message: "Error occurred while booking the ticket",
                error: err.message,
            })
            .code(500);
    }
...
const getAllTickets = async (req, h) => {
    try {
        ...
        return h
            .response({
                message: "Error occurred while fetching tickets",
                error: err.message,
            })
            .code(500);
    }
...
const getTicketsByUserId = async (req, h) => {
    try {
        ...
        if (!tickets.length) {
            return h
                .response({ message: "No tickets found for this user" })
                .code(404);
        }
        ...
        return h
            .response({
                message: "Error occurred while fetching tickets for the user",
                error: err.message,
            })
            .code(500);
    }
...
const cancelTicket = async (req, h) => {
    try {
        ...
        await ticket.remove();
        return h
            .response({ message: "Ticket successfully cancelled" })
            .code(200);
    } catch (err) {
        return h
            .response({
                message: "Error occurred while cancelling the ticket",
                error: err.message,
            })
            .code(500);
    }
